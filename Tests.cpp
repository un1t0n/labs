...
//Тест на повышение стоимости товара

bool Tests::test_markup(Goods* test_good) { //Тестируем загрузки денег в автомат
...
//Тест на понижение стоимости товара
...
//Тест на процентную наценку товара
...
//Тест на строковое представление товара
...
//Тест на перегрузку добавления товаров
...
//Тест на перегрузку уменьшения количества товаров
...
//Тест на приведение к типу double
...
//тест на проверку исключения стоимости
...
//тест присвоения имени объекту класса ChildGood

bool Tests::test_name_of_ChildGood(ChildGoods* test_good) {
	test_good->setName("Apple");
	const char* expect = "Apple";
	const char* real;
	real = test_good->getName();
...
//тест присвоения возрастной категории объекту класса ChildGood

bool Tests::test_age_of_ChildGood(ChildGoods* test_good) {
	test_good->setAge(15);
	int expect = 15;
	int real;
	real = test_good->getAge();
	if (expect == real) {
		successful++;
		return true;
	}
...
//тест исключения в возрастной категории объекту класса ChildGood

bool Tests::test_not_true_age_of_ChildGood(ChildGoods* test_good) {
	test_good->setAge(-1);
	int expect = 15;
	int real;
	real = test_good->getAge();
...
//тест на строковое представление объекта класса ChildGood

bool Tests::test_string_of_ChildGood(ChildGoods* test_good) {
	test_good->setType("Constructor");
	test_good->setAge(10);
	test_good->setDate(5,5,1999);
	test_good->setCost(10.0);
	test_good->setAmount(100);
	const char* expect = "Constructor 10 Apple 5 5 1999 10.0 100";
	const char* real;
	real = test_good->strObj();
...
//тест на проверку возрастной категории объекта класса ChildGood

bool Tests::test_age_category_of_ChildGood(ChildGoods& test_good){
	test_good.setAge(10);
	if (test_good.age <= 18) {
		successful++;
		return true;
...
//тест присвоения имени объекту класса MilkGood

bool Tests::test_name_of_MilkGood(MilkGoods* test_good) {
	test_good->setType("Apple");
	const char* expect = "Apple";
	const char* real;
	real = test_good->getName();
...
//тест строкового представления объекта класса MilkGood

bool Tests::test_string_of_MilkGood(MilkGoods* test_good) {
	test_good->setType("Chesse");
	test_good->setDate(5,5,1999);
	test_good->setCost(10.0);
	test_good->setAmount(100);
	const char* expect = "Chesse 5 5 1999 Apple 25 5 2001 10.0 100";
	const char* real;
	real = test_good->strObj();
...
//тест на понижение стоимости после срока годности объекта класса MilkGoods

bool Tests::test_delay_of_MilkGood(MilkGoods& test_good) {
	test_good.setMilkDate(4,5,1999);
	test_good.setDate(5,5,1999);
	test_good.delay(test_good.milkday,test_good.day);
	int expect = 5;
	int real = test_good.getCost();
	if (expect == real) {
		successful++;
...
//тест на исключение даты объекта класса MilkGoods

bool Tests::test_date_of_MilkGood(MilkGoods* test_good) { 
	test_good->setDate(0,0,0);
	int expectDay = 5;
	int expectMonth = 5;
	int expectYear = 1999;
